...
#include <algorithm>
#include <pthread.h>
#include <iostream>// g++ transE.cpp -o transE -pthread -O3 -march=native
#include <sstream>//  transE
#include <random>

using namespace std;
const float pi = 3.141592653589793238462643383;

int transeThreads = 30;
int transeTrainTimes = 5000;
int nbatches = 10;
int dimension = 50;
...
unsigned long randd(int id) {
    // Generate the next random number
    previous = (1664525 * previous + 1013904223) % 4294967296;
    return previous;
}

int rand_max(int id, int x)
{
	uint32_t res = randd(id) % x;
	return res;
}
...
long relationTotal, entityTotal, tripleTotal;
...
{
	float sum = 0;
	uint64_t last1 = e1 * dimension;
	uint64_t last2 = e2 * dimension;
	uint64_t lastr = rel * dimension;
	for (int ii = 0; ii < dimension; ii++)
	{
...
int corrupt_head(int id, int h, int r)
{
	...
	rr = lef;
	int tmp = rand_max(id, entityTotal);
