...
def convert_format(entity):
    value = re.sub(r'[^a-zA-Z0-9 ]', '', entity)
    return value


def construct_potential_kb(potential_tuples, kb):
    potent_kb = POTENT_KB()
    entity_list = set([convert_format(potent_tuple[2]) for potent_tuple in potential_tuples])
    entity_list.update(set([convert_format(potent_tuple[0]) for potent_tuple in potential_tuples]))
    for entity in entity_list:
        for key, value in kb.entities.items():
            if key == entity:
                potent_kb.entities[key] = value

    # get all entity from potent kb
    key_list = []
    for key, value in potent_kb.entities.items():
        key_list.append(key)

    for line in potential_tuples:
        pot_head = convert_format(line[0])
        pot_tail = convert_format(line[2])
        if pot_head in key_list and pot_tail in key_list:
            potent_kb.add_relation(pot_head, line[1], pot_tail, [{'start': 0, 'end': 0}])

    return potent_kb


def select_optional_model(nlp_model):
    with open(f"./data_preparation/datasets/{nlp_model}/model_comparison.csv", "r") as file:
...
def link_predication(nlp_model, kge_model_list, entity, relation, number, original_kb, extracted_kb, kb):
    ...
    if os.path.exists(best_model_file_path):
        ...
        with open(f"./data_preparation/datasets/{nlp_model}/predicated_result_kg.csv", "w") as file:
        ...
        for kge_model in kge_model_list:
            ...
            pd.DataFrame.from_dict(data=dataset.ent2id, orient='index').to_csv(
                f"./data_preparation/datasets/{nlp_model}/entity_id.csv",
                header=False)
            pd.DataFrame.from_dict(data=dataset.rel2id, orient='index').to_csv(
                f"./data_preparation/datasets/{nlp_model}/rel_id.csv",
                header=False)
    ...
    outperform_model = select_optional_model(nlp_model)
